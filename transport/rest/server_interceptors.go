		...
		if origin := ctx.Request.Header.Get("Origin"); origin != "" {
			...
			ctx.Writer.Header().Set(
				...
				"Origin,X-Requested-With,Content-Type,Content-Length,"+
					"Accept-Encoding,X-CSRF-Token,Accept,Authorization,AccessToken,Token",
			)
			ctx.Writer.Header().Set(
				"Access-Control-Expose-Headers",
				"Access-Control-Allow-Origin,Access-Control-Allow-Headers,"+
					"Cache-Control,Content-Type,Content-Length,Content-Language",
			)
	...
	return func(ctx *gin.Context) {
		defer func() {
			...
			if isBrokenPipe(r) {
				...
				_ = ctx.Error(errors.New(StringBrokenPipe))
				ctx.Abort()
			} else {
...
func isBrokenPipe(r any) bool {
	...
	if opErr, ok := r.(*net.OpError); ok {
		...
		if errors.As(opErr.Err, &syscallErr) {
			if strings.Contains(strings.ToLower(syscallErr.Error()), StringBrokenPipe) ||
				strings.Contains(strings.ToLower(syscallErr.Error()), StringConnectionReset) {
				brokenPipe = true
			}
		}
	}
